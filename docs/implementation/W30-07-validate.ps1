...
$work = Join-Path ([IO.Path]::GetTempPath()) ('4-me-not-W30-07-run-' + [guid]::NewGuid().ToString('N'))
Expand-Archive -LiteralPath $SourceZip -DestinationPath $work
$specRoot = Join-Path $work '4-me-not-collab-v0.4'
$publishedWork = Join-Path $work 'published'
Expand-Archive -LiteralPath $SourceZip -DestinationPath $publishedWork
$publishedRoot = Join-Path $publishedWork '4-me-not-collab-v0.4'
...
$overlayFiles = @(
    ...
    'contracts/authority-boundary.schema.json',
    'contracts/authority_boundary.py',
    'docs/authority-boundary.md',
    'fixtures/w30-04-authority-cases.json',
    'tests/test_w30_04_authority_boundary.py',
    'docs/w30-05-mapping-audit.md',
    'tests/test_w30_05_mapping.py',
    'docs/migration.md',
    'contracts/migration_examples.py',
    'tests/test_w30_06_migration.py',
...
try {
    ...
    try {
        ...
        foreach ($run in @(
            @{name='published'; cwd=$publishedRoot; argv=@('-X','utf8','-m','unittest','discover','-s','tests','-p','test_*.py','-v')},
            @{name='active'; cwd=$specRoot; argv=@('-X','utf8','-m','unittest','discover','-s','tests','-p','test_*.py','-v')},
            @{name='baselines'; cwd=$specRoot; argv=@('scripts/check_baselines.py')}
        )) {
            $arguments = $run.argv
            Push-Location $run.cwd
            try {
                $output = @(& $Python @arguments 2>&1 | ForEach-Object { "$_" })
                $code = $LASTEXITCODE
            } finally { Pop-Location }
            $log = Join-Path $PSScriptRoot ('W30-07-' + $run.name + '.log')
            ...
            $results += [ordered]@{
                name=$run.name; working_directory=$run.cwd; executable=$Python; arguments=$arguments
...
$record = [ordered]@{
    task='W30-07'; timestamp=(Get-Date -Format o); repo=$repo; spec_root=$specRoot
...
$record | ConvertTo-Json -Depth 8 | Set-Content -LiteralPath (Join-Path $PSScriptRoot 'W30-07-validation.json') -Encoding utf8
