...
$work = Join-Path ([IO.Path]::GetTempPath()) ('4-me-not-W30-02-run-' + [guid]::NewGuid().ToString('N'))
...
try {
    ...
    try {
        ...
        foreach ($run in @(
            ...
            $code = $LASTEXITCODE
            $log = Join-Path $PSScriptRoot ('W30-02-' + $run.name + '.log')
...
$record = [ordered]@{
    task='W30-02'; timestamp=(Get-Date -Format o); repo=$repo; spec_root=$specRoot
...
$record | ConvertTo-Json -Depth 8 | Set-Content -LiteralPath (Join-Path $PSScriptRoot 'W30-02-validation.json') -Encoding utf8
